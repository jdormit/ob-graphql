...
This project is available [via Melpa](https://melpa.org/#/ob-graphql). You can install it via `M-x package-install <RET> ob-graphql` or using the package management solution of your choice. Alternatively, you can put `ob-graphql.el` on your load path somewhere and load it via `(require 'ob-graphql)`. After that you'll be able to evaluate GraphQL code blocks.
